        ...
        for row in csv.reader(f):
            ...
            try:
                ts = datetime.strptime(
                    row[0].strip() + " " + row[1].strip(), "%Y-%m-%d %H:%M:%S"
                )
                hz_low = float(row[2])
                hz_step = float(row[4])
...
def plot_heatmap(grid, times, freqs_mhz, outpath):
    ...
    cbar = fig.colorbar(
        plt.cm.ScalarMappable(
            norm=plt.Normalize(vmin=vmin, vmax=vmax), cmap="inferno"
        ), ax=ax, pad=0.02
    )
...
def plot_spectrum(freqs_mhz, means, maxs, noise, peaks, outpath):
    ...
    ax1.fill_between(freqs_mhz, means, noise, where=(means > noise),
                     color="#ff6b35", alpha=0.3)
    ax1.plot(freqs_mhz, means, color="#ff8c55", linewidth=0.85, label="Mean power")
    ax1.plot(freqs_mhz, maxs,  color="#ffd700", linewidth=0.5, alpha=0.5,
             label="Peak power")
    ax1.axhline(noise,      color="#888",    linewidth=0.8, linestyle="--",
                label=f"Noise floor ({noise:.1f} dBm)")
    ax1.axhline(noise + 6,  color="#44aaff", linewidth=0.7, linestyle=":",
    ...
    for idx in peaks:
        ...
        ax1.annotate(
            f"{f:.2f}", xy=(f, m), xytext=(0, 8), textcoords="offset points",
            ha="center", va="bottom", fontsize=6.5, color=col, rotation=70,
        )
    ...
    for f, snr, col in zip(fmhz, snrs, cols):
        ax2.text(f, snr + 0.2, f"{f:.2f}", ha="center", va="bottom",
                 fontsize=6.5, color="white", rotation=70)
...
def plot_full_report(grid, times, freqs_mhz, means, maxs, stds, noise, peaks,
                     outpath):
    BG = "#0d1117"
    fig = plt.figure(figsize=(18, 14), facecolor=BG)
    gs = gridspec.GridSpec(3, 1, figure=fig, height_ratios=[1.6, 1.2, 1.0],
                           hspace=0.38)
    ...
    for idx in peaks:
        snr = means[idx] - noise
        _, col = tier(snr)
        ax_wf.axhline(freqs_mhz[idx], color=col, linewidth=0.6, alpha=0.5,
                      linestyle="--")

    # Mean spectrum
    ax_sp.fill_between(freqs_mhz, means, noise, where=(means > noise),
                       color="#ff6b35", alpha=0.3)
    ax_sp.plot(freqs_mhz, means, color="#ff8c55", linewidth=0.85,
               label="Mean power")
    ax_sp.plot(freqs_mhz, maxs, color="#ffd700", linewidth=0.5, alpha=0.5,
               label="Peak power")
    ax_sp.axhline(noise,     color="#888",    linewidth=0.8, linestyle="--",
                  label=f"Noise  {noise:.1f} dBm")
    ax_sp.axhline(noise + 6, color="#44aaff", linewidth=0.7, linestyle=":",
                  label="+6 dB threshold")
    for idx in peaks:
        f = freqs_mhz[idx]
        m = means[idx]
        ...
        ax_sp.annotate(
            f"{f:.2f}", xy=(f, m), xytext=(0, yoff),
            textcoords="offset points", ha="center", va="bottom",
            fontsize=6, color=col, rotation=70,
        )
    ax_sp.set_xlim(freqs_mhz[0], freqs_mhz[-1])
    ax_sp.set_ylabel("Power (dBm)", color="#cccccc", fontsize=10)
    ax_sp.legend(loc="upper left", fontsize=7.5, facecolor="#1a1a2e",
    ...
    for f, snr, col in zip(fmhz, snrs, cols):
        ax_snr.text(f, snr + 0.15, f"{f:.2f}", ha="center", va="bottom",
                    fontsize=5.8, color="white", rotation=70)
    for level, col in [(6, "#44aaff"), (14, "#ffaa00"), (20, "#ff2244")]:
    ...
    ax_snr.legend(handles=patches, loc="upper right", fontsize=7.5,
...
def build_summary(freqs_hz, means, maxs, stds, noise, peaks, times,
                  csv_path, threshold_db):
    signals = []
    for idx in peaks:
        f = freqs_hz[idx] / 1e6
        m = float(means[idx])
        ...
        tname, _ = tier(snr)
        stability = (
            "stable" if sd < 1.5 else
            "variable" if sd < 3.0 else
            "intermittent"
        )
        signals.append({
            "freq_mhz": round(f, 3),
...
def main():
    ...
    parser.add_argument("--prefix",    default="",
                        help="Filename prefix for output charts")
    ...
    plot_spectrum(freqs_mhz, means, maxs, noise, peaks,
                  outdir / f"{stem}_spectrum.png")
    plot_full_report(grid, times, freqs_mhz, means, maxs, stds, noise, peaks,
                     outdir / f"{stem}_report.png")

    summary = build_summary(freqs_a, means, maxs, stds, noise, peaks, times,
                            csv_path, args.threshold)
